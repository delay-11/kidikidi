...
function hasRequiredUserInfo() {
  if (typeof validateUserInfo === "function") {
    return validateUserInfo(false);
  }
  ...
  return hasName && hasPhone && hasOrder && hasEmail;
}
...
helpIcons.forEach((icon) => {
  icon.addEventListener("click", (e) => {
    ...
    if (isQuestionTooltip && !hasRequiredUserInfo()) {
      ...
      if (typeof setFormNotice === "function") {
        setFormNotice(
          "주문자명, 핸드폰번호, 주문번호, 이메일 입력 후 ? 안내를 확인해주세요.",
          "error",
        );
      }
